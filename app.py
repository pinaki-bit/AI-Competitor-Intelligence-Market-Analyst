...
from agents import get_market_scores, run_analysis, run_mock_analysis
from history_store import append_history, clear_history, load_history
...
with st.sidebar:
    ...
    st.caption(
        "Stored in process memory only — not persisted. "
    ...
    if gemini_present and tavily_present:
        st.markdown(
            "<div style='color:#10b981;font-size:0.78rem;'>✓ Both API keys detected</div>",
            unsafe_allow_html=True,
        )
    else:
        missing = []
    ...
    with st.expander("🔧 Model override (advanced)"):
        st.caption(
            "Leave blank to use the auto-detected model. Override with any "
            "model name your Gemini key has access to. If the chosen model "
            "is unavailable, the app will automatically try a fallback."
        )
        fast_override = st.text_input(
            "Quick-tier model", value="", placeholder="e.g. gemini-2.0-flash"
        )
        pro_override = st.text_input("Deep Dive model", value="", placeholder="e.g. gemini-2.5-pro")
    ...
    demo_mode = st.checkbox(
        "Demo / Simulation Mode", value=False, help="Full simulation — no API keys needed"
    )

    st.markdown(
        "<hr style='border-color:rgba(255,255,255,0.05);margin:1.2rem 0;'>", unsafe_allow_html=True
    )

    st.markdown("### 🕑 Analysis History")
    if not st.session_state.history:
        st.markdown(
            "<div style='color:#475569;font-size:0.83rem;'>No analyses yet.</div>",
            unsafe_allow_html=True,
        )
    else:
...
if st.session_state.report_markdown and st.button(
    "\U0001f5d1\ufe0f Clear & New Analysis", use_container_width=False
):
...
if run_btn:
    ...
    if not topic_clean:
        st.warning("⚠️  Please enter a company name or product niche.")
        ...
        def status_cb(agent, msg):
            st.session_state.agent_steps.append(
                {"agent": agent, "msg": msg, "time": datetime.now().strftime("%H:%M:%S")}
            )
        ...
        try:
            if demo_mode:
                ...
                report, mode = run_analysis(
                    topic=topic_clean, depth=research_depth, status_callback=status_cb
                )
...
if st.session_state.report_markdown:
    ...
    with h_col2:
        ...
        try:
            ...
            generate_pdf_from_markdown(
                st.session_state.report_markdown, st.session_state.topic_analyzed, pdf_path
            )
            with open(pdf_path, "rb") as f:
                pdf_bytes = f.read()
            st.download_button(
                "📥 PDF Report", data=pdf_bytes, file_name=pdf_fn, mime="application/pdf"
            )
        except Exception as e:
    ...
    if scores:
        st.markdown(
            "<div class='section-title' style='margin-top:1.2rem;'>📊 Market Intelligence Dashboard</div>",
            unsafe_allow_html=True,
        )
        st.caption(
            "Scores are illustrative — derived from the report content. "
        ...
        score_items = [
            (
                ...
                "#10b981",
                "How strong is the market growth potential?",
            ),
            (
                "Competitive Pressure",
                scores["competitive_pressure"],
                "#f59e0b",
                "How intense is competition in this space?",
            ),
            (
                "Growth Trajectory",
                scores["growth_trajectory"],
                "#6366f1",
                "Momentum and expansion rate of the company",
            ),
            (
                "Innovation Score",
            ...
            (
                ...
                scores["risk_level"],
                "#ef4444",
                "Overall strategic and market risk exposure",
            ),
            (
                "Market Maturity",
                scores["market_maturity"],
                "#0ea5e9",
                "How mature/saturated is the target market?",
            ),
        ]
    ...
    if st.session_state.agent_steps and not st.session_state.loaded_from_history:
        st.markdown(
            "<div class='section-title'>🤖 Agent Activity Log</div>", unsafe_allow_html=True
        )
        agent_icons = {"Researcher": "🔬", "Analyst": "⚖️", "Writer": "✍️"}
        agent_colors = {"Researcher": "#6366f1", "Analyst": "#a855f7", "Writer": "#ec4899"}
        cards = ""
